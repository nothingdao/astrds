...
import { buildCrankLiquidityTransaction, fetchLiquidityCrankState } from '../app/src/lib/spaceVault'
...
async function main() {
  ...
  console.log('pendingLamports', state.pendingLamports, 'pendingSol', state.pendingSol)
  console.log('positionExists', state.positionExists, 'positionLocked', state.positionLocked)
  ...
  built.transaction.instructions.forEach((ix, i) => {
    ...
    ix.keys.forEach((k, j) => console.log(j, k.pubkey.toBase58(), 'w', k.isWritable, 's', k.isSigner))
  })
