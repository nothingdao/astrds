...
export type BudgetDistribution = 'even' | 'random' | 'early' | 'late' | 'manual'

export type CurvePolicy =
  | { mode: 'fixed'; value: number }
  | { mode: 'linear'; from: number; to: number }
  | { mode: 'step'; start: number; increment: number; cap?: number }
  | { mode: 'randomRange'; min: number; max: number; seedMode?: 'session' | 'level' }
...
export const DEFAULT_PROGRESSION_BANDS: LevelBandPolicy[] = [
  {
    ...
    shipPickups: { mode: 'budget', count: 30, distribution: 'manual', levels: Array.from({ length: 30 }, (_, i) => i + 1) },
    powerups: { mode: 'budget', count: 30, distribution: 'manual', levels: Array.from({ length: 30 }, (_, i) => i + 1) },
...
export function getBandForLevel(bands: LevelBandPolicy[] | undefined, level: number): LevelBandPolicy {
  const sorted = [...(bands?.length ? bands : DEFAULT_PROGRESSION_BANDS)].sort((a, b) => a.fromLevel - b.fromLevel)
  return sorted.find((band) => level >= band.fromLevel && level <= band.toLevel) ?? sorted[sorted.length - 1]
}

export function resolveCurve(policy: CurvePolicy, level: number, fromLevel: number, toLevel: number, seed = ''): number {
  ...
  switch (policy.mode) {
    ...
    case 'randomRange': {
      const key = policy.seedMode === 'session' ? `${seed}:${fromLevel}-${toLevel}` : `${seed}:${level}`
      return policy.min + (policy.max - policy.min) * seeded01(key)
    }
  }
}

export function budgetLevels(policy: BudgetPolicy, fromLevel: number, toLevel: number, seed = ''): number[] {
  ...
  if (policy.distribution === 'manual' && policy.levels?.length) {
    return [...new Set(policy.levels.filter((level) => level >= fromLevel && level <= toLevel))].slice(0, count)
  }
...
export function resolveLevelConfig(
  ...
  return {
    ...
    powerupBudget: powerupLevels.filter((candidate) => candidate === level).length,
