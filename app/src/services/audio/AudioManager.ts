...
import { SOUND_MAP, type SfxBucketId, type StingerPlaylistId, type LevelBand } from './SoundMap'
...
class AudioManager {
  ...
  start() {
    ...
    this.unsubscribers.push(
      audioService.on('musicEnded', ({ trackId }) => this.handleMusicEnded(trackId))
    )
  ...
  playFromSfxBucket(bucketId: SfxBucketId) {
    const id = this.nextFromBucket(bucketId, SOUND_MAP.sfxBuckets[bucketId], this.sfxBucketStates)
    if (id) audioService.playSound(id)
  }

  async playFromStingerPlaylist(playlistId: StingerPlaylistId) {
    const id = this.nextFromBucket(playlistId, SOUND_MAP.stingerPlaylists[playlistId], this.stingerPlaylistStates)
    if (!id) return
    ...
    await audioService.duckMusic(SOUND_MAP.stingerDuck.targetVolume, SOUND_MAP.stingerDuck.duckMs)
  ...
  private handleStateChange(state: MachineState) {
    ...
    if (state === MachineState.PAUSED) {
      ...
      audioService.duckMusic(SOUND_MAP.stingerDuck.targetVolume, SOUND_MAP.stingerDuck.duckMs)
    } else if (this.pauseDucked) {
      this.pauseDucked = false
      if (this.stingersPlaying === 0) audioService.restoreMusic(SOUND_MAP.stingerDuck.restoreMs)
    }
  ...
  private playLevelBand(level: number) {
    const band = SOUND_MAP.levelBands.find((b) => level >= b.from && level <= b.to)
    ...
    if (!sameBand) {
      ...
      this.currentPlaylistState = {
        order: band.playlist.length === 1 ? [...band.playlist] : shuffle(band.playlist),
        index: 0,
      }
    }

    if (!this.currentPlaylistState) return
    const trackId = this.currentPlaylistState.order[this.currentPlaylistState.index] ?? band.playlist[0]
  ...
  private handleMusicEnded(trackId: string) {
    ...
    if (!this.currentPlaylistState || this.currentPlaylistState.index >= this.currentPlaylistState.order.length) {
      this.currentPlaylistState = { order: shuffle(band.playlist), index: 0 }
    }

    const next = this.currentPlaylistState.order[this.currentPlaylistState.index]
    this.currentPlaylistState.index++
    this.currentTrackId = null
  ...
  private restoreAfterStingersIfNeeded() {
    ...
    const restore = () => audioService.restoreMusic(SOUND_MAP.stingerDuck.restoreMs)
    if (SOUND_MAP.stingerDuck.holdMs > 0) {
      this.stingerRestoreTimer = setTimeout(restore, SOUND_MAP.stingerDuck.holdMs)
    } else {
      restore()
    }
  }
