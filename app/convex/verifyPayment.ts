...
const SPACE_VAULT_PROGRAM_ID = new PublicKey('4bRZK8XfziVhLCgvtRdFJyTgN6tXGSPJT8xfbtt1AxBB')
...
const accountKeyToString = (key: unknown): string | null => {
  ...
  if (key && typeof key === 'object') {
    ...
    if (pubkey && typeof pubkey === 'object' && 'toString' in pubkey) return String(pubkey)
    if ('toString' in key) return String(key)
  }
  return null
}

const getInstructionProgramId = (ix: Record<string, unknown>, accountKeys: unknown[]): string | null => {
  if (typeof ix.programId === 'string') return ix.programId
  if (ix.programId && typeof ix.programId === 'object' && 'toString' in ix.programId) return String(ix.programId)
  if (typeof ix.programIdIndex === 'number') return accountKeyToString(accountKeys[ix.programIdIndex])
  return null
}
...
export const verifyPayment = action({
  ...
  handler: async (ctx, { txSignature, walletAddress, paymentType }) => {
    if (paymentType !== 'SOL') throw new Error('ASTRDS payment is no longer supported')

    const existing = await ctx.runQuery(internal.sessions.getByTxSignature, { txSignature })
    ...
    if (feePayer !== wallet.toBase58()) throw new Error('Payment fee payer does not match wallet')

    const signerMatches = accountKeys.some((key) => {
      if (!key || typeof key !== 'object') return false
      const record = key as { pubkey?: unknown; signer?: unknown }
      return record.signer === true && accountKeyToString(record.pubkey ?? key) === wallet.toBase58()
    })
    // Non-json transaction responses do not expose the signer flag; fee-payer match is still required.
    if (accountKeys.some((key) => key && typeof key === 'object' && 'signer' in key) && !signerMatches) {
    ...
    for (const ix of instructions) {
      if (getInstructionProgramId(ix, accountKeys) !== SPACE_VAULT_PROGRAM_ID.toBase58()) continue
    ...
    if (paidLamports === null) throw new Error('Transaction did not call game_payment')
    ...
    if (!Number.isFinite(quarterUsd) || quarterUsd <= 0 || !Number.isFinite(solUsdPrice) || solUsdPrice <= 0) {
      throw new Error('Unable to determine required payment amount')
    }
    const requiredLamports = BigInt(Math.ceil((quarterUsd / solUsdPrice) * 1e9))
    if (paidLamports < requiredLamports) {
      throw new Error(`Payment too small: paid ${paidLamports.toString()} lamports, required ${requiredLamports.toString()}`)
    }
