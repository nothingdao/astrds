...
async function getTokenMetadata(mintAddress: string, rpcEndpoint: string) {
  try {
    ...
    return {
      ...
      logoUri: (asset.content?.links?.image ?? asset.content?.files?.[0]?.uri) as string | undefined,
    }
  } catch {
    return null
  }
...
export const vaultHealthCheck = action({
  ...
  handler: async (ctx) => {
    ...
    const convexDeposits = await ctx.runQuery(internal.spaceDeposits.getAllDeposits, {})
    ...
    const mismatched: { onChain: typeof onChainPools[0]; convexRemaining: number; convexStatus: string }[] = []
    ...
    for (const pool of onChainPools) {
      ...
      if (!convex) {
        ...
        mismatched.push({ onChain: pool, convexRemaining: convex.remainingAmount, convexStatus: convex.status })
      } else {
        healthy.push(pool)
      }
    }
...
export const syncVaultToConvex = action({
  ...
  handler: async (ctx, { poolAddresses }) => {
    ...
    const connection = new Connection(rpcEndpoint, 'confirmed')
    const results: { address: string; symbol: string; action: string; error?: string }[] = []
    ...
    for (const address of poolAddresses) {
      try {
        ...
        if (!accountInfo) {
          results.push({ address, symbol: '?', action: 'skipped', error: 'Account not found on-chain' })
          continue
        }

        const pool = parseDepositPool(pubkey, Buffer.from(accountInfo.data as Buffer))
        const meta = await getTokenMetadata(pool.mint, rpcEndpoint)

        const mintInfo = await connection.getAccountInfo(new PublicKey(pool.mint), 'confirmed')
        const programId = mintInfo?.owner.equals(TOKEN_2022_PROGRAM_ID) ? 'TOKEN_2022' : 'TOKEN'

        const decimals = meta?.decimals ?? 6
        const tokensPerPill = 100 * Math.pow(10, decimals)

        const result = await ctx.runMutation(internal.spaceDeposits.upsertRecoveredDeposit, {
          poolAddress: address,
        ...
        results.push({ address, symbol: '?', action: 'error', error: err instanceof Error ? err.message : String(err) })
      }
    }
