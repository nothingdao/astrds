...
export const consumeSession = internalMutation({
  ...
  handler: async (ctx, { walletAddress }) => {
    const session = await ctx.db
      ...
      .order('desc')
      .first()
    if (!session || session.expiresAt <= Date.now() || session.consumed === true) {
...
const requireServerAuth = (request: Request): Response | null => {
  const apiKey = process.env.ADMIN_API_KEY
  if (!apiKey) return new Response('Not configured', { status: 503, headers: CORS_HEADERS })
...
export const consumeSessionHttp = httpAction(async (ctx, request) => {
  ...
  if (typeof walletAddress !== 'string') return new Response('Invalid body', { status: 400, headers: CORS_HEADERS })

  const result = await ctx.runMutation(internal.sessions.consumeSession, { walletAddress })
